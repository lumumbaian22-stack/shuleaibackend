...
router.post('/teachers', authorize('admin','super_admin'), async (req,res)=>{
  try { const email=req.body.email || `teacher-${Date.now()}@test.local`; const user=await User.create({ name:`${req.body.firstName||'Test'} ${req.body.lastName||'Teacher'}`.trim(), email, phone:req.body.phone, password:req.body.password || 'Test@12345', role:'teacher', schoolCode:schoolCode(req), preferences:{testRunId:req.body.testRunId||null} }); const row=await Teacher.create({ userId:user.id, classId:req.body.classId || null, subjects:req.body.subject ? [req.body.subject] : [], classTeacher:req.body.classTeacher ? 'yes' : null, approvalStatus:'approved' }); return ok(res,{...row.toJSON(), User:user.getPublicProfile?.() || user},'Teacher created'); } catch(e){ return fail(res,500,e.message); }
});
router.post('/parents', authorize('admin','super_admin'), async (req,res)=>{
  try { const email=req.body.email || `parent-${Date.now()}@test.local`; const user=await User.create({ name:`${req.body.firstName||'Test'} ${req.body.lastName||'Parent'}`.trim(), email, phone:req.body.phone, password:req.body.password || 'Test@12345', role:'parent', schoolCode:schoolCode(req), preferences:{testRunId:req.body.testRunId||null} }); const row=await Parent.create({ userId:user.id, relationship:req.body.relationship || 'guardian' }); return ok(res,{...row.toJSON(), User:user.getPublicProfile?.() || user},'Parent created'); } catch(e){ return fail(res,500,e.message); }
});
router.post('/students', authorize('admin','teacher','super_admin'), async (req,res)=>{
  try { const email=req.body.email || `${(req.body.admissionNumber||Date.now()).toString().toLowerCase()}@student.test.local`; const user=await User.create({ name:`${req.body.firstName||'Test'} ${req.body.lastName||'Student'}`.trim(), email, phone:req.body.phone, password:req.body.password || 'Test@12345', role:'student', schoolCode:schoolCode(req), preferences:{testRunId:req.body.testRunId||null} }); const row=await Student.create({ userId:user.id, elimuid:req.body.admissionNumber || undefined, grade:req.body.className || req.body.grade || 'Not Assigned', gender:normalizeGender(req.body.gender), parentName:req.body.parentName, parentEmail:req.body.parentEmail, parentPhone:req.body.parentPhone, preferences:{testRunId:req.body.testRunId||null, curriculum:req.body.curriculum||null} }); return ok(res,{...row.toJSON(), User:user.getPublicProfile?.() || user},'Student created'); } catch(e){ return fail(res,500,e.message); }
});
