...
async function uploadToCloudinary({ buffer, mimeType, originalName, schoolCode, kind, checksum }) {
  ...
  const publicId = `${Date.now()}-${String(checksum || crypto.randomUUID()).slice(0, 18)}`;
...
async function uploadPersistentObject(options) {
  ...
  if (provider === 'cloudinary') {
    try {
      ...
      if (process.env.CLOUDINARY_REQUIRED === 'true' || process.env.REQUIRE_OBJECT_STORAGE === 'true') throw error;
