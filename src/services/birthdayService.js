...
const { Student, User, Parent, StudentParent, Teacher, TeacherSubjectAssignment, Class, School, Alert, BirthdayEvent } = require('../models');
...
async function recipients(student,schoolCode,settings={}){
  ...
  const allow={student:configured.student!==false,parent:configured.parent!==false,teacher:configured.teacher!==false,admin:configured.admin!==false};
  const ids=new Set();
  ...
  if(allow.student&&settings.announceToClass===true&&cls){const classmates=await Student.findAll({where:{classId:cls.id,status:'active'},attributes:['userId']});classmates.forEach(s=>ids.add(s.userId));}
...
async function processSchool(schoolCode,{now=new Date(),createdBy=null}={}){
  ...
  const students=await (Student.unscoped?Student.unscoped():Student).findAll({where:{status:'active',dateOfBirth:{[Op.ne]:null},...(suppressedIds.length?{id:{[Op.notIn]:suppressedIds}}:{})},include:[{model:User,where:{schoolCode,role:'student',isActive:true},attributes:['id','name','schoolCode']} ]});
  let created=0;
  for(const student of students){const birthday=nextBirthday(student.dateOfBirth,now,timezone);if(!birthday)continue;const birthdayDate=ymd(birthday,timezone);const days=Math.round((new Date(`${birthdayDate}T12:00:00Z`)-new Date(`${today}T12:00:00Z`))/86400000);if(!advanceDays.includes(days))continue;const eventType=days===0?'same_day':`advance_${days}`;const [event,isNew]=await BirthdayEvent.findOrCreate({where:{schoolCode,studentId:student.id,eventDate:birthdayDate,eventType},defaults:{schoolCode,studentId:student.id,eventDate:birthdayDate,eventType,status:'created',audience:{},createdBy,metadata:{daysBefore:days}}});if(!isNew)continue;
    ...
    for(const userId of target.userIds){const u=await User.findByPk(userId,{attributes:['id','role']});if(!u)continue;await Alert.create({userId:u.id,role:u.role,type:'system',severity:'info',title,message,categoryLabel:'Birthday',sourceType:'birthday',sourceLabel:'Shule AI birthday reminder',targetRole:u.role,targetUserId:u.id,studentId:student.id,classId:target.classId,priority:'normal',dedupeKey:`birthday:${student.id}:${birthdayDate}:${eventType}:${u.id}`,data:{schoolCode,studentId:student.id,birthdayDate,daysBefore:days,age,className:target.className}});}
