...
async function deleteOrArchiveStructure({ user, id }) {
  ...
  if (!structure) throw new Error('Fee structure not found');
  const before = structure.toJSON();
  const feeCount = await Fee.count({ where: { schoolCode, feeStructureId: String(structure.id) } });
  const paymentCount = await Payment.count({ where: { schoolCode, feeStructureId: String(structure.id) } }).catch(() => 0);
