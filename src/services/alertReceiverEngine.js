...
async function getAlertsForUser(user, { studentId = null, limit = 120, calendarOnly = false, upcomingOnly = false } = {}) {
  const where = {
    [Op.or]: [
      ...
      sequelize.literal(`COALESCE("Alert"."data", '{}'::jsonb) @> '{"targetRoles":["${normalizeRole(user.role)}"]}'::jsonb`),
      sequelize.literal(`COALESCE("Alert"."data", '{}'::jsonb) @> '{"receiverRoles":["${normalizeRole(user.role)}"]}'::jsonb`)
    ]
  };
  const rows = await Alert.findAll({ where, order: [['createdAt', 'DESC']], limit: Number(limit || 120) }).catch(async () => {
    return Alert.findAll({ where: { userId: user.id }, order: [['createdAt', 'DESC']], limit: Number(limit || 120) });
  });
  const safe = [];
