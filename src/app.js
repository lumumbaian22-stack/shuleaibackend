...
async function ensureCriticalDashboardColumns(req, res, next) {
  ...
  try {
    ...
    await sequelize.query('ALTER TABLE IF EXISTS "SchoolPaymentRequests" ADD COLUMN IF NOT EXISTS "subscriptionStartDate" TIMESTAMP WITH TIME ZONE').catch(() => null);
    await sequelize.query('ALTER TABLE IF EXISTS "SchoolPaymentRequests" ADD COLUMN IF NOT EXISTS "subscriptionEndDate" TIMESTAMP WITH TIME ZONE').catch(() => null);
    ...
    await sequelize.query('ALTER TABLE IF EXISTS "Users" ALTER COLUMN "signature" TYPE TEXT').catch(() => null);
