const { Fee, Payment, Student, Parent } = require('../models');
...
exports.getFees = async (req, res) => {
  try {
    ...
    const fees = await Fee.findAll({ where: { studentId }, order: [['year', 'DESC'], ['term', 'DESC']] });
  ...
  try {
    ...
    if (!fee) {
      fee = await Fee.create({
        ...
        term, year,
        totalAmount: 5000, // default, can be set by school
        paidAmount: 0,
        status: 'unpaid'
      });
    }
