...
// ---------- PUBLIC API ANALYTICS ROUTE COMPATIBILITY ----------
// These handlers back /api/analytics/* routes. They intentionally use conservative
// queries so the backend boots even when optional production data is still sparse.
exports.getClassAnalytics = async (req, res) => {
    try {
        ...
        const schoolCode = req.user.schoolCode || classItem.schoolCode;
        if (req.user.role !== 'super_admin' && schoolCode && classItem.schoolCode !== schoolCode) {
            return res.status(403).json({ success: false, message: 'You cannot view analytics for another school.' });
        }

        const students = await Student.findAll({
            where: { grade: classItem.grade },
            include: [{ model: User, where: { schoolCode: classItem.schoolCode, role: 'student' }, attributes: ['id', 'name'] }]
        });
        const studentIds = students.map(s => s.id);

        const records = studentIds.length ? await AcademicRecord.findAll({ where: { studentId: { [Op.in]: studentIds } } }) : [];
        const attendance = studentIds.length ? await Attendance.findAll({ where: { studentId: { [Op.in]: studentIds } } }) : [];

        const classAverage = records.length ? Math.round(records.reduce((sum, r) => sum + (Number(r.score) || 0), 0) / records.length) : 0;
        const present = attendance.filter(a => a.status === 'present').length;
        const attendanceRate = attendance.length ? Math.round((present / attendance.length) * 100) : 0;
        ...
        const studentPerformance = students.map(student => {
            ...
            return { studentId: student.id, name: student.User?.name || student.elimuid, average, records: own.length };
        }).sort((a, b) => b.average - a.average);
        ...
        res.json({
            ...
            data: {
                ...
                overview: { studentCount: students.length, classAverage, attendanceRate, recordCount: records.length },
                subjectAverages,
                studentPerformance
            }
        });
...
exports.getSchoolAnalytics = async (req, res) => {
    try {
        const schoolCode = req.user.schoolCode || req.query.schoolCode;
        if (!schoolCode && req.user.role !== 'super_admin') return res.status(400).json({ success: false, message: 'schoolCode required' });

        const whereSchool = schoolCode ? { schoolCode } : {};
        const totalClasses = await Class.count({ where: { ...whereSchool, isActive: true } });
        const totalStudents = await Student.count({ include: [{ model: User, where: { ...whereSchool, role: 'student' } }] });
        const totalTeachers = await Teacher.count({ include: [{ model: User, where: { ...whereSchool, role: 'teacher' } }] });
        const totalParents = await Parent.count({ include: [{ model: User, where: { ...whereSchool, role: 'parent' } }] });

        const records = await AcademicRecord.findAll({ where: schoolCode ? { schoolCode } : {} });
        const averageScore = records.length ? Math.round(records.reduce((sum, r) => sum + (Number(r.score) || 0), 0) / records.length) : 0;

        const thirtyDaysAgo = moment().subtract(30, 'days').format('YYYY-MM-DD');
        const attendance = await Attendance.findAll({ where: schoolCode ? { schoolCode, date: { [Op.gte]: thirtyDaysAgo } } : { date: { [Op.gte]: thirtyDaysAgo } } });
        ...
        res.json({
            success: true,
            data: {
                overview: { totalStudents, totalTeachers, totalParents, totalClasses, averageScore, attendanceRate },
                subjectPerformance,
...
exports.compareCurriculum = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'super_admin' && schoolCode && student.User?.schoolCode !== schoolCode) {
            return res.status(403).json({ success: false, message: 'You cannot compare analytics for another school.' });
        }

        const records = await AcademicRecord.findAll({ where: { studentId: student.id } });
        const average = records.length ? Math.round(records.reduce((sum, r) => sum + (Number(r.score) || 0), 0) / records.length) : 0;
        const subjects = [...new Set(records.map(r => r.subject).filter(Boolean))];

        const subjectReadiness = subjects.map(subject => {
            const own = records.filter(r => r.subject === subject);
            const score = own.length ? Math.round(own.reduce((sum, r) => sum + (Number(r.score) || 0), 0) / own.length) : 0;
            return {
                subject,
                score,
                status: score >= 80 ? 'exceeding' : score >= 60 ? 'meeting' : score >= 40 ? 'approaching' : 'needs_support'
            };
        });
        ...
        res.json({
            ...
            data: {
                ...
                overall: { average, records: records.length },
                subjectReadiness,
                recommendation: average >= 70
                    ? 'Student is broadly on track. Keep strengthening weaker subject areas.'
                    : 'Student needs targeted support and parent/teacher follow-up in low-scoring subjects.'
            }
        });
    } catch (error) {
