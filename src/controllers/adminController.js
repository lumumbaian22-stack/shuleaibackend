...
// ============ V66 ACADEMIC AUDIT + UNLOCK ============
exports.getMarksAuditLog = async (req, res) => {
  try {
    const { AcademicRecord, Student, User, Teacher } = require('../models');
    const { term, year, studentId, subject } = req.query || {};
    ...
    const records = await AcademicRecord.findAll({
      where,
      include: [
        { model: Student, include: [{ model: User, attributes: ['id','name','profileImage'] }] },
        { model: Teacher, include: [{ model: User, attributes: ['id','name','profileImage'] }] }
      ],
      order: [['updatedAt','DESC']],
      limit: 300
    });
    const data = records.map(r => ({
      id: r.id, studentId: r.studentId, studentName: r.Student?.User?.name || 'Student', teacherName: r.Teacher?.User?.name || 'Teacher',
      subject: r.subject, assessmentType: r.assessmentType, assessmentName: r.assessmentName, score: r.score, grade: r.grade,
      term: r.term, year: r.year, status: r.status, isPublished: r.isPublished, lockedAt: r.lockedAt,
      auditTrail: Array.isArray(r.auditTrail) ? r.auditTrail : []
    }));
    res.json({ success:true, data });
  } catch(error) { console.error('V66 audit log error:', error); res.status(500).json({ success:false, message:error.message }); }
};

exports.unlockAcademicRecord = async (req, res) => {
  try {
    const { AcademicRecord } = require('../models');
    const { recordId } = req.params;
    const reason = req.body?.reason || 'Admin academic correction unlock';
    const record = await AcademicRecord.findOne({ where:{ id:recordId, schoolCode:req.user.schoolCode } });
    ...
    trail.push({ action:'mark_unlocked', by:req.user.id, reason, at:new Date().toISOString(), oldStatus:record.status });
    await record.update({ status:'draft', isPublished:false, unlockedBy:req.user.id, unlockReason:reason, auditTrail:trail });
    res.json({ success:true, message:'Mark unlocked. Teacher may correct it; all edits remain audit logged.', data:record });
  } catch(error) { console.error('V66 unlock mark error:', error); res.status(500).json({ success:false, message:error.message }); }
};
