...
test('runtime schema repair is disabled by default in production', () => {
  const server = fs.readFileSync(path.join(root, 'server.js'), 'utf8');
  assert.match(server, /ALLOW_RUNTIME_SCHEMA_REPAIR/);
  assert.match(server, /Runtime schema repair disabled/);
});
