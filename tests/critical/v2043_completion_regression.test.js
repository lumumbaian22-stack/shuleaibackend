...
test('schema repair and detailed diagnostics require super-admin authentication', () => {
  const app = readBackend('src/app.js');
  assert.match(app, /app\.get\('\/api\/health\/detailed', protect,[\s\S]{0,120}authorize\('super_admin'\)/);
  assert.match(app, /app\.post\('\/api\/system\/repair-schema', protect,[\s\S]{0,120}authorize\('super_admin'\)/);
});

test('destructive seed refuses production and requires explicit development opt-in', () => {
  const seed = readBackend('src/utils/seed.js');
  assert.match(seed, /NODE_ENV === 'production'/);
  assert.match(seed, /ALLOW_DESTRUCTIVE_SEED/);
  assert.match(seed, /sync\(\{ force: true \}\)/);
});
...
test('dependency lock has the hardened archive overrides', () => {
  ...
  assert.equal(pkg.version, '2.1.538');
