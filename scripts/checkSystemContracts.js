...
const build = '2047-grounded-system-repair';
...
if (process.argv.includes('--write')) {
  fs.writeFileSync(path.join(root, 'backend', 'ROUTE_MANIFEST.json'), `${JSON.stringify({ build, count: routes.length, routes }, null, 2)}\n`);
  fs.writeFileSync(path.join(root, 'frontend', 'API_CALL_MANIFEST.json'), `${JSON.stringify({ build, count: apiCalls.length, apiCalls }, null, 2)}\n`);
}
