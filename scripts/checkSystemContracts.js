...
if (process.argv.includes('--write')) {
  fs.writeFileSync(path.join(root, 'backend', 'ROUTE_MANIFEST.json'), `${JSON.stringify({ build: '2039-production-migration-recovery-lock', count: routes.length, routes }, null, 2)}\n`);
  fs.writeFileSync(path.join(root, 'frontend', 'API_CALL_MANIFEST.json'), `${JSON.stringify({ build: '2039-production-migration-recovery-lock', count: apiCalls.length, apiCalls }, null, 2)}\n`);
}
